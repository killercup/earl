...
use parser::{to_u32, Context, Parser, SchemeType, ViolationFn};
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(
            ...
            "ParseOptions {{ base_url: {:?}, encoding_override: {:?}, \
             violation_fn: {:?} }}",
            self.base_url, self.encoding_override, self.violation_fn
        )
    }
}
...
impl Url {
    ...
    {
        Ok(HostAndPort {
            host: self.host().ok_or(()).or_else(|()| io_error("URL has no host"))?,
            port: self
...
fn path_to_file_url_segments(
    ...
    for component in path.components().skip(1) {
        ...
        serialization.push('/');
        serialization
            .extend(percent_encode(component.as_os_str().as_bytes(), PATH_SEGMENT_ENCODE_SET));
    }
    if empty {
...
fn file_url_segments_to_pathbuf(
    ...
    let mut bytes = if cfg!(target_os = "redox") { b"file:".to_vec() } else { Vec::new() };
    ...
    debug_assert!(path.is_absolute(), "to_file_path() failed to produce an absolute Path");
    Ok(path)
}
