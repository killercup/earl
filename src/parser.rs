        ...
        match scheme_type {
            ...
            SchemeType::SpecialNotFile => {
                ...
                self.violation_fn.call_if(ExpectedDoubleSlash, || {
                    input.clone().take_while(|&c| matches!(c, '/' | '\\')).collect::<String>()
                        != "//"
                });
        ...
        match first_char {
            None => {
                if let Some(base_url) = base_file_url {
                    ...
                    Ok(Url { serialization: self.serialization, fragment_start: None, ..*base_url })
                } else {
    ...
    pub fn parse_cannot_be_a_base_path<'i>(&mut self, mut input: Input<'i>) -> Input<'i> {
        loop {
            ...
            match input.next_utf8() {
                ...
                Some((c, utf8_c)) => {
                    self.check_url_code_point(c, &input);
                    self.serialization.extend(utf8_percent_encode(utf8_c, SIMPLE_ENCODE_SET));
                }
                None => return input,
            }
        }
